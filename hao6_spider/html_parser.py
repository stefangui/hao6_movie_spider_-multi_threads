...
class HtmlParser(object):
    ...
    def get_movie_items(self, page_url, html_cout):
        ...
        soup = BeautifulSoup(html_cout, 'html.parser')
        links = soup.find('ul', attrs={'class': "list"})
        all_url_links = links.find_all('li')
        counts = 0
        for link in all_url_links:
            movie_node = link.find('a')
            item_movie_url = movie_node.get("href")
            movie_content = movie_node.get_text()
            movie_title = movie_node.get_text()
